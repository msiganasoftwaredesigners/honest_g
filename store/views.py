...
def get_price_and_colors(request):
    ...
    if size_variation_id is not None:
        try:
            ...
            kare = size_variation.kare
            colors = [color.name for color in size_variation.color.all()]
            
            return JsonResponse({'price': price, 'colors': colors, 'kare': kare})
...
# def products_by_location(request, location):
#     products = Product.objects.filter(product_location__iexact=location)
#     return render(request, 'products_by_location.html', {
#         'location': location,
#         'products': products
#     })

def products_by_location(request, category, location):
    products = Product.objects.filter(
        product_location__iexact=location,
